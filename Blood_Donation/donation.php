...
$donorResult = $conn->query("SELECT * FROM donors WHERE normal_user_id = '$userId' AND donation_req_status = 'confirm' ");
...
elseif($userType=="organaization"){
    echo "<script>
    alert('Organization not register for this');
    ...
    echo "<script>
    alert('Please register for Donor');
    window.location.href = 'blood_home.php';
</script>";
    exit();
}
